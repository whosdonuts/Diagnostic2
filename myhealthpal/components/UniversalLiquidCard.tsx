...
interface UniversalLiquidCardProps {
  ...
  /** Extra NativeWind classes merged onto the outer wrapper */
  className?: string;
  /** Adds press-scale spring on touch */
  pressable?: boolean;
 ...
 * UniversalLiquidCard
 *
 * A platform-adaptive frosted-glass container that uses CSS
 * `backdrop-filter` on web and `expo-blur` BlurView on native.
 *
 * Wrap any content in this card to get the Liquid Glass aesthetic
 * with optional spring-physics press feedback.
 */
export function UniversalLiquidCard({
  ...
  if (Platform.OS === "web") {
    return (
      ...
      >
        {/* Specular inner glow — CSS radial gradient (web only) */}
        <div
          style={{
            position: "absolute",
            inset: 0,
  ...
  /* ─── Native: expo-blur provides the real gaussian blur ─── */
  return (
    <Animated.View
    ...
    >
      ...
      />
      {/* Specular inner glow — top-edge highlight on native */}
      <View
...
const nativeStyles = StyleSheet.create({
  ...
  innerGlow: {
    ...
    left: 0,
    right: 0,
    bottom: "50%" as unknown as number,
